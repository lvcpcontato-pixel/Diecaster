...
const SYNC_URL_KEY = 'diecast_sheets_sync_url';
const DEFAULT_SYNC_URL = 'https://script.google.com/macros/s/AKfycbzhTn9t-q6UfUncD65H_2txU8HRAECP8_vqAA1yr7zFZ1YAZEx8U0oH7RXCmR2oFk5NxQ/exec';
...
export const saveCar = async (car: Car): Promise<Car[]> => {
  ...
  if (syncUrl) {
    try {
      // Enviamos como texto plano para evitar preflight de CORS que o Google Script não gosta
      await fetch(syncUrl, {
        method: 'POST',
        body: JSON.stringify({ action: 'upsert', car })
      });
    } catch (e) {
      console.error("Erro ao sincronizar foto/dados:", e);
    }
  }
  return newCars;
};
...
export const pushAllToCloud = async (cars: Car[]): Promise<void> => {
  const syncUrl = getSyncUrl();
  if (!syncUrl) return;
  // Enviamos um por um para garantir que as fotos (base64) não estourem o limite do POST
...
export const clearDatabase = async (): Promise<void> => localStorage.removeItem(STORAGE_KEY);
